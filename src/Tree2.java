...
public class Tree2 {
    ...
    public static void main(String[] args) {
        ...
        for(int i = 1; i<= N; i++){
            ...
            distance[1]=0;
            first_flag =false;
            int pathSrcCount[] = dijkstra(matrix,1,i);
            int n1= distance[i];
            Arrays.fill(distance, Integer.MAX_VALUE);
            distance[i]=n1;
            Arrays.fill(visited_vertex,false);
            ...
            first_flag =true;
            int pathIntermediateCount[] = dijkstra(matrix, i, N);
            int n2 = distance[N];
            if(n2==Integer.MAX_VALUE) {
                System.out.println("-1");
                System.out.println("path Count" + 0);

            }
            else {
                System.out.println(n2);
                System.out.println("path Count" + (pathSrcCount[i] * pathIntermediateCount[N]));
    ...
    static int[] dijkstra(int[][] matrix, int source,int destination)
    {
            ...
            if (n >= 1 && n <= N){
                ...
                for (int j = 1; j <= N; j++) {
                    ...
                    if ((last_vertex || first_flag || j!= N)&&!visited_vertex[j] && matrix[n][j] != 0 && distance[n] != Integer.MAX_VALUE && distance[n] + matrix[n][j] + wating < distance[j]) {
                        ...
                        parent[j]=n;
                        pathCount[j] = 1;
                    } else if ((last_vertex || first_flag || j!= N)&&!visited_vertex[j] && matrix[n][j] != 0 && distance[n] != Integer.MAX_VALUE && distance[n] + matrix[n][j] + wating == distance[j]){
                        pathCount[j]++;
                    }
                }
            }
